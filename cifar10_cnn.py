...
def predict_image(img_path, model, display=True):
    """
    ...
    try:
        ...
        print(f"\nPrediction Results:")
        print(f"  Predicted class: {predicted_class} ({predicted_class_ua})")
        print(f"  Confidence: {confidence:.4f} ({confidence * 100:.2f}%)")
        print(f"\nAll class probabilities:")
        for i, prob in enumerate(prediction[0]):
            print(f"  {class_names[i]:12s}: {prob:.4f} ({prob * 100:.2f}%)")
...
print("predict_image('your_image.jpg', model)")
print("\nExample: predict_image('dog3.jpg', model)")
